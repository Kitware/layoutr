<template>
  <v-app>
    <v-navigation-drawer fixed app class="pa-4">
      ...
      <v-checkbox v-model="showEdges" label="Show edges" hide-details></v-checkbox>
      <under-one-slider v-model="edgeOpacity" :min="0.01" label="Edge opacity" />
      <under-one-slider v-model="size" :min="0.01" label="Size" />
      <field-box v-model="sizeField" :items="['None', ...fields]" label="Size field" />
      <v-btn block @click="toggleLayout">{{ layoutRunning ? 'Stop' : 'Start' }} layout</v-btn>
      <under-one-slider v-model="alpha" :read-only="this.layoutRunning" label="Energy" />
      <app-slider v-model="chargeStrength" :min="0" :max="50" :step="1" label="Charge strength" />
      <app-slider v-model="theta" :min="0.5" :max="3.0" :step="0.1" label="Charge approximation" />
      <under-one-slider v-model="collideStrength" label="Collide force" />
      <under-one-slider v-model="linkStrength" label="Link force" />
      <v-checkbox v-model="center" label="Center force" hide-details />
      <under-one-slider v-model="xStrength" label="X force" />
      <field-box v-model="xField" :items="fields" label="X field" />
      <under-one-slider v-model="yStrength" label="Y force" />
      <field-box v-model="yField" :items="fields" label="Y field" />
      <under-one-slider v-model="radialStrength" label="Radial force" />
      <field-box v-model="radialField" :items="fields" label="Radial field" />
...
import UnderOneSlider from "./components/UnderOneSlider";
...
export default {
  ...
  mounted() {
    ...
    if (graphURL) {
      fetch(graphURL)
        .then(response =>
          Promise.all([response.text(), response.headers.get("Content-Type")])
        )
        .then(([content, contentType]) => {
          if (contentType.startsWith("application/json")) {
            ...
            console.log(
              `Error: Cannot handle files with content type ${contentType}.`
            );
          }
        });
    }
    ...
    layoutWorker.onmessage = e => {
      if (e.data.type === "graph") {
        ...
        lines = layer
          .createFeature("line")
          .data(graph.edges.map(e => [nodeMap[e.source], nodeMap[e.target]]))
          .style({
        ...
        points = layer
          .createFeature("point", {
            ...
            style: {
              strokeColor: "black",
              fillColor: nodeid =>
                graph.nodes[nodeid].select
                  ? ["yellow", "red"][graph.nodes[nodeid].select - 1]
                  : "grey",
              fillOpacity: 0.5,
              strokeOpacity: 0.5
            },
  ...
  methods: {
      ...
      if (file) {
        ...
        reader.onload = function(evt) {
          ...
          if (extension === "json") {
            ...
            console.log(
              `Error: Cannot handle files with extension ${extension}.`
            );
          }
        };
    ...
    updateSizeScale() {
      if (points) {
        const sizeScale = scales.generateSizeScale(
          graph.nodes,
          this.sizeField,
          this.size
        );
